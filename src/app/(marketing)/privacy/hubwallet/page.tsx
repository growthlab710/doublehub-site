  ...
  return (
    ...
    >
      ...
      <ul>
        ...
        <li>
          サービス改善のため、次のアプリ利用状況に限り、当社自身による最小限の記録を行います。記録する内容はいずれも、発生日時・画面や導線の種類・アプリのバージョン、および課金状態と突き合わせるための
          RevenueCat
          の匿名利用者識別子であり、当社が管理するサーバーへ送信・保存されます。氏名・メールアドレス・家計データ（金額・店名・カテゴリ・メモ・取引記録やレシート画像など）は送信しません。
          <ul>
      ...
      <ul>
        ...
        <li>
          ...
          プラン・任意機能）の提供開始に伴い改定。保存先（Supabase・東京リージョン）、同期対象・対象外データ、暗号化とアクセス制御、停止・解約時の保持方針、アカウント削除によるクラウドデータ消去を追記（第2.1項・第4項・第5項・第8項・第9項）
        </li>
      </ul>
    </PrivacyLayout>
  );
