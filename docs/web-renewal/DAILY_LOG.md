...
## 2026-05-09 — Support フォームの「お問い合わせ対象」を未選択+必須化

### 背景

サポートページ /support/ の「お問い合わせ対象」が DoubleHub をデフォルト
選択していたため、TrainNote / BookCompass / HubWallet のユーザーが
アプリからリンクで遷移してきたまま気付かずに送信し、
DoubleHub 宛として届いてしまう誤送信ケースが発生する懸念があった。

### 変更内容（方針 A: 未選択 + 必須化）

`src/app/(marketing)/support/_components/SupportForm.tsx`:

- 対象サービスの初期値を `SERVICES[0]`（DoubleHub）→ 空文字列に変更。
  select の先頭に `<option value="" disabled>-- 選択してください --</option>`
  を置いて未選択状態を示す。
- ラベルに必須マーク `*`（accent-warm 色）と「ご利用中のアプリを必ず
  選択してください。」の説明文を追加。
- 送信ハンドラで service が空のときに送信をブロックし、
  - select を accent-warm の枠線に切り替え、
  - 直下に role=alert の赤色エラーメッセージを表示、
  - select に focus + scrollIntoView してユーザーをガイド。
- 値が選ばれたら自動的にエラー UI を解除。
- aria-invalid / aria-describedby も付与してスクリーンリーダーにも伝える。

「お問い合わせ種別」と詳細以外の任意項目は変更なし（種別のデフォルトは
誤って届いても運用上の致命傷ではないため、UI の追加負担を避けた）。

### 検証

- `pnpm build` 成功。
- ローカル dev サーバーで以下を目視確認:
  - 初期表示で対象サービスが「-- 選択してください --」になり、必須マーク
    と説明文が出る。
  - 詳細を入れて未選択のまま送信 → 赤枠 + 「お問い合わせ対象を選択して
    ください。」のエラー表示、select に focus。
  - サービスを選ぶと UI が即座に通常状態に戻る。


## 2026-05-15 — HubWallet プロダクトページ更新（CSV 記述削除・¥380/月・比較表・15 秒紹介動画）

### 変更内容

`src/app/(marketing)/products/hubwallet/page.tsx`:

- 差別化セクション（自動連携型 行）の「OCR・手入力・音声・CSV のみ」から
  CSV を削除し「OCR・手入力・音声のみ」に変更。MVP リリースでは CSV
  インポート未提供のため。
- Privacy セクションのチェック項目「銀行・カード連携なし。OCR・手入力・
  音声・CSV のみ」からも CSV を削除。
- FAQ「銀行やクレジットカードと連携できますか？」の回答から
  「CSV インポート」記述を削除。
- FAQ「既存の家計簿アプリから移行できますか？」の項目自体を削除
  （CSV インポート対応予定が立っていないため）。
- Plus プラン価格を `¥480／月（仮） + 年額 ¥3,800（仮）` から
  `¥380／月` に変更。年額プランの記載は完全に撤去。
- Plans セクション下に Free と Plus の機能比較表を新設。比較項目は
  添付の huranBi-Jiao-2-3.jpg をソースに以下を反映:
  - AI OCR・音声・カテゴリ推定: Free 月 10 回 / Plus 月 500 回
  - 手入力: ✓ / ✓
  - 予算管理: 2 カテゴリ / 無制限
  - サブカテゴリ追加: — / 30 件
  - 定期支出: ✓ / ✓
  - レポート機能: ✓ / ✓
  - 親カテゴリ別レポート: — / ✓
- Free / Plus プランカードの説明文と機能箇条書きも同表に合わせて更新
  （CSV 箇条書きを除去、Plus に「予算管理は無制限カテゴリ」「サブカテゴリ
  追加（最大 30 件）」「親カテゴリ別レポート」を追記）。
- App Screenshots セクション冒頭に 15 秒の紹介動画
  (`/videos/hubwallet-product-intro.mp4`) を `<video controls muted
  autoPlay loop playsInline preload="metadata">` で配置。poster は
  `/images/hubwallet-screen-home.jpg`。9:19 の縦型枠で既存スクリーン
  ショットの見え方と整合。
- スクリーンショット差し替え:
  - `/images/hubwallet-screen-triage.jpg` ← 添付 suwaihuShi-Yi-2.jpg
    （仕分け — スワイプで「修正・保留・確定」）
  - `/images/hubwallet-screen-monthly.jpg` ← 添付 rehoto-5.jpg
    （月次レポート — 6ヶ月推移とカテゴリ別の進捗）
  - `/images/hubwallet-screen-voice.jpg` ← 添付 Yin-Sheng-Ru-Li-4.jpg
    （音声入力 — 話しかけて未整理に保存）
- 比較表用に `PlanCheck` 補助コンポーネントをファイル内に追加
  （on/off と accent の 2 値で ✓ / ―を表示）。

`public/images/`, `public/videos/`:

- 上記スクリーンショット 3 枚を同名上書き。
- `hubwallet-product-intro.mp4` を新規追加（約 8 MB、15 秒）。

### 検証

- `pnpm install --frozen-lockfile` 成功。
- `pnpm build` 成功（Next.js 16.2.4 / Turbopack、53 ページ生成）。
- `/products/hubwallet` 含む全静的ページが従来通り SSG された
  ことを確認。CSV 文字列は同ページから完全に消えていることを grep で確認。

---

## 2026-05-15 — HubWallet ホーム/年間ビュー画像とアプリアイコン差し替え

### 概要

ユーザー追加リクエストに対応し、HubWallet のホーム画面・年間レポートのスクリーンショットを新版に差し替え、新しいアプリアイコン (AppIcon-3.jpg) をプロダクトページ・トップページの動線に反映した。

### 変更ファイル

`public/images/`:

- `hubwallet-screen-home.jpg` ← 添付 `homuHua-Mian.jpg`
  （Hero のスマホ枠、紹介動画 poster、ホームスクリーンショットすべてに反映）
- `hubwallet-screen-yearly.jpg` ← 添付 `rehotoXia-Duan-2.jpg`
  （App Screenshots の「年間ビュー — 支出見込みと来月予測」）
- `hubwallet-app-icon.jpg` ← 添付 `AppIcon-3.jpg` を新規追加

`src/app/(marketing)/products/hubwallet/page.tsx`:

- Hero バッジの 💰 絵文字を撤去し、`/images/hubwallet-app-icon.jpg`
  を `next/image` で表示する squircle 風アイコンに置き換え。

`src/lib/site/config.ts`:

- `products` 配列の `hubwallet` エントリに
  `appIcon: '/images/hubwallet-app-icon.jpg'` を追加。
  `MarketingHeader` の `ProductMenuIcon` がこれを拾い、PC ドロップ
  ダウン / モバイルメニューでもアプリアイコンが表示される。

`src/components/marketing/ProductCards.tsx`:

- `productIconMap` に `hubwallet` を追加（現状トップグリッドからは
  filter 済みだが、将来表示する場合の整合性を確保）。

`src/components/marketing/EcosystemTabs.tsx`:

- `panelIconMap.finance` を絵文字 💰 から画像版
  `/images/hubwallet-app-icon.jpg` に変更。トップページの
  Ecosystem タブで HubWallet ボタンに公式アイコンが表示される。

`src/app/(marketing)/app-linking/page.tsx`:

- HubWallet 行の 💰 絵文字バッジを `next/image` のアイコン表示に置換
  （他プロダクトと同じ 56x56 squircle）。

### 検証

- `pnpm install` 成功。
- `pnpm build` 成功（Next.js 16.2.4 / Turbopack、53 ページ生成）。
- 前回コミットの変更（CSV 言及削除・¥380/月・比較表・スクリーンショット
  /動画）は保持されていることを確認。

## 2026-05-16 — Book Compass プロダクト誘導記事を2本追加（reading-log-what-to-record / reading-habit-log-review）

### 背景

Book Compass の App Store 公開を機に、検索流入からアプリ DL に
つなげる SEO/プロダクト誘導記事を増やす。今回は読書記録アプリ
選定と読書習慣化という、Book Compass の主要なコンバージョン
経路に近い2クエリを狙った。HubWallet は App Store 審査中のため
記事内では言及していない。

### 追加ファイル

`content/blog/reading-log-what-to-record.mdx`:

- 「読書記録アプリで何を残すべき？続く人の記録項目を整理」
- 主キーワード「読書記録アプリ 記録項目」を想定。読了日・一言の
  気づき・短いメモ・また読み返したい理由の4項目に絞る考え方を
  解説し、Book Compass プロダクトページ／App Store へ誘導。
- 出典として app-liv（読書記録アプリ特集）と my-best（比較記事）
  を本文に引用。

`content/blog/reading-habit-log-review.mdx`:

- 「読書習慣が続かない人へ。記録と振り返りで読み続ける方法」
- 主キーワード「読書習慣 続かない」を想定。意志ではなく
  「軽く記録／可視化／振り返り」の3点セットで読書を継続する方法を
  整理し、Book Compass の本棚・記録・AI 対話で支える流れに接続。
- 出典として allisone のリーディング習慣に関する記事、my-best の
  読書記録アプリ比較を本文に引用。

### 整合

- 両記事ともリポジトリ既存の MDX 規約（純 Markdown、frontmatter
  に title/description/publishedAt/updatedAt/category/slug/
  readingTime/summary/tags/faq）に準拠。
- カテゴリは既存の "読書"（bookcompass-reading-memo と同じ）を
  使用し、関連記事スコアリング（同カテゴリ加点）でクラスター化
  されるようにした。
- 内部リンクは reading-ai-habit / bookcompass-reading-memo /
  reading-log-what-to-record ⇄ reading-habit-log-review と相互に
  張り、`/products/bookcompass/` への CTA をリード・終盤・関連
  記事の各所に配置。
- App Store URL は `siteConfig.social.appStoreBookCompass` と同じ
  id6760604663 を本文末で明示。
- 公開日は JST 2026-05-16。
...
## 2026-07-03 (JST) — ブログ記事追加（上半期の半年レビュー）※本日3本目

ブランチ: `feature/add-blog-half-year-review`（main へ `--no-ff` マージ）

### 実施内容

- **ブログ記事「上半期の自分をAIと振り返る——記録データから『半年レビュー』を作る方法」を追加**
  - `content/blog/half-year-review-with-ai.mdx`（カテゴリ: ライフデータ×自己理解）
  - 7月の折り返しタイミングに合わせた evergreen 記事。直近がAIニュース続きだったため
    プロダクト導線記事でバランスを調整。TrainNote / BookCompass / HubWallet / DoubleHub の
    4プロダクトページと既存記事4本への内部リンクあり。
  - 構成: 記憶での振り返りの歪み → 素材集め → AIへの問いの立て方（要約でなく変化と例外）
    → 下半期は「足す」より「守る」→ DoubleHubの方向性への接続。

### 検証

- `pnpm build` 成功（60ページ静的生成 / 型・ビルドエラーなし）。

---

## 2026-07-06 (JST) — DoubleHub プライバシーポリシーに日記（フォトダイアリー）を追記

### 実施内容

- `/privacy/doublehub` に日記機能の取り扱いを追記（DoubleHub Ver.2.0.0 対応）。
  - §1 取得情報: 日記の写真・気分・メモ（写真は端末のみ保存）を追加
  - §2 利用目的: 日記の記録・見返し・AI 一言コメント・状況読み取り・傾向学習を追加
  - §3 保存先と外部送信: **写真は端末＋iCloud バックアップのみ／当方サーバーに保存しない。
    AI の一言時のみ縮小画像を一時送信し当方・Gemini とも保存しない**旨を明記。
    メモ・気分・状況テキストは他 AI 機能と同様に送信され得る旨を追記
  - §5 ユーザーによる管理: 日記削除で端末内写真も削除される旨を追記
  - `lastUpdated` を 2026-07-06 に更新

### 判断・注意

- origin/main には HubWallet 連携の記載がなく、本追記も **HubWallet を含めていない**
  （local main に未 push の HubWallet 追記コミット `4ae2870` が存在したが、HubWallet は
  アプリ未公開のため今回は取り込まず、クリーンな origin/main 基点に日記のみを追加）。
  HubWallet 連携が公開される際に `4ae2870` 相当の追記を別途行う。
...
## 2026-07-11 (JST) — BookCompass プライバシーポリシー更新

ブランチ: `docs/bookcompass-privacy-audio`

### 実施内容

- `/privacy/bookcompass/` を BookCompass 2.0.0 の内省ノート・音声入力仕様に更新。
  - §1 取得情報: 内省ノート本文を追加し、音声データ自体は当方で保存しないことを明記
  - §2 利用目的: 内省ノートと音声認識による文字入力を追加
  - §3 保存先と外部送信: クラウドDBへ正式保存する対象、端末内下書き、Apple 音声認識の
    オンデバイス／サーバー経路、音声データ非保存、内省ノートを現時点で AI 分析へ
    送信しないことを明記
  - §5 ユーザー管理: ノート削除、端末内下書き、マイク・音声認識権限の変更方法を追加
  - ページ概要と `lastUpdated` を 2026-07-11 に更新

### 検証

- `pnpm typecheck` 成功（型エラーなし）。
- `pnpm build` 成功（61ページ生成、`/privacy/bookcompass` の静的生成を確認）。
- ローカル配信ページへ HTTP 200 でアクセスし、本文・更新日・Apple プライバシーポリシーへのリンクを確認。

---

## 2026-07-11 (JST) — BookCompass プライバシーポリシーの事業者名一般化

ブランチ: `docs/bookcompass-privacy-provider-generalization`

### 実施内容

- 公開ポリシーからクラウドDB製品名とサーバー実装名を削除し、以下の一般表現へ変更。
  - 「クラウドデータベース・認証基盤」
  - 「クラウドバックエンド上のサーバー処理」
- 取得データ、利用目的、国外データセンターで処理・保存される可能性は引き続き明記。
- 第三者提供欄を事業者カテゴリ表記に変更し、委託先へ適切なデータ保護を求める旨を追加。
- 音声送信先として説明が必要なAppleと、ユーザーコンテンツを処理するAI事業者の記載は維持。
...
## 2026-07-16 (JST) その2 — ブログ: 思想記事シリーズ4本を掲載（カテゴリ「思想」新設）

ブランチ: `feature/add-philosophy-articles`

### 背景

- Note公開済みの思想記事シリーズ「AI時代の分かれ道」①〜③のブログ版（在庫）と、続編①（お金＝自己紹介）をサイトへ掲載。以後の運用は「ブログ先出し→Note転載」（2026-07-16のマーケセッションで決定。思想記事のみ両載せ、実用記事はブログ専用）。

### 実施内容

- 新規4記事（カテゴリ「思想」）:
  - `ai-two-types-of-information`（① 情報には2種類ある）series: AI時代の分かれ道
  - `information-fatigue`（② 情報疲れの正体）同series。原稿の誤字（「調べればå」）を修正して掲載
  - `grow-your-own-ai`（③ もう一人の自分を育てる）同series
  - `money-self-introduction`（続編① お金＝自己紹介）series: 責めない・観察するテクノロジー
- シリーズナビ・次回予告を相互リンク化。③④の【アプリ紹介記事リンク】は `/products/doublehub/`・`/products/hubwallet/` へ差し替え。Note用フッター・ハッシュタグはブログ向けに調整

### 検証

- `pnpm build` 成功（EXIT=0・静的生成 67/67、新規4ページ生成を確認）。

---

## 2026-07-16 (JST) その3 — ブログトップのUI改善（新着の視認性＋連載シリーズ圧縮）

ブランチ: `feature/update-blog-index-ui`

### 背景

- 連載シリーズカード（全話リスト表示・2列×2段）が縦に大きく、最近の投稿がスクロールしないと見えない／新着かどうかがパッと見で分からない、という課題（ユーザー指摘）への対応。

### 実施内容

- `src/lib/content/series-meta.ts` 新規: シリーズ名→1行キャッチ＋説明のメタ定義（4シリーズ分。未定義は第1回のdescriptionにフォールバック）
- `SeriesSection.tsx` をコンパクト化: 全話リストを廃止し「キャッチ＋シリーズ名＋説明（2行clamp）＋全N回・最新日付・第1回から読む」の説明カードに変更。md以上3カラム1段、モバイルは横スクロール（snap）。カードのリンク先は第1回
- `BlogExplorer.tsx`: 記事カードに **NEWバッジ**（公開14日以内）を追加、日付をカード上部右へ移動、直近14日は「今日/昨日/N日前」の相対表記。判定は閲覧時刻ベース（マウント後に有効化しhydrationミスマッチと静的ビルドの鮮度ズレを回避）

### 検証

- `pnpm build` 成功（EXIT=0・静的生成 67/67）。

---

## 2026-07-16 (JST) その4 — シリーズ説明文の見切れ修正
...
## 2026-07-25 (JST) — TrainNote プライバシーポリシーの写真・ビジュアルスコア対応

ブランチ: `feature/update-trainnote-privacy-visualscore`

### 背景

- TrainNote v3.2.0 (build 2) の App Store 提出準備中に、**公開中のポリシーが 2026-03-25 版のまま**で、写真関連の記載が一切ないことが判明。
- アプリ側は v3.0.0 でボディフォト（進捗写真ライブラリ）と AIボディ変化レポート（写真を外部AIへ送信）を、今回さらに **ビジュアルスコア**（写真のAI採点）をリリースする状態で、実態とポリシーが乖離していた。
- App Store ガイドライン 5.1.1 / 5.1.2（センシティブデータの開示）および App Privacy 表記との整合上、提出前の是正が必要と判断。
- アプリ側リポジトリ（TrainNote）の `docs/index.html`（2026-06-20 版）には写真条項が入っていたが**未公開**だったため、その内容＋ビジュアルスコア条項をこちらへ反映した。

### 実施内容

`src/app/(marketing)/privacy/trainnote/page.tsx` を更新（2026-03-25 → 2026-07-25）。

- **1. 取得する情報**: 進捗写真ライブラリ／ボディ変化レポートの写真、ビジュアルスコアの採点結果（部位ごとのスコア・基準写真の指定・採点日時）を追加。アプリ設定に体脂肪率を追記
- **2. 利用目的**: 進捗写真ライブラリの提供・ボディ変化レポート作成、ビジュアルスコアの採点／推移表示を追加
- **3. 保存先と外部送信**: 写真はアプリ専用領域に保存され iOS写真ライブラリへ自動保存されない旨、レポートAI生成時の送信範囲、写真の永続保存をしない旨、共有カードに実写真を含めない旨を追加。加えてビジュアルスコアについて「**ユーザーが採点を実行したときのみ**写真を送信」「撮影・保存だけでは送信されない」「**初回に同意画面**を表示し同意時のみ採点」「スコアは端末内保存・他ユーザーと共有／比較しない」を明記
- **5. ユーザーによる管理**: レポート作成またはビジュアルスコア採点を実行しない限り写真は外部送信されない旨を追加
- **6. 健康関連情報**: レポート／ビジュアルスコアは見た目の変化の目安であり、筋肉量・体脂肪率・身体サイズの測定や健康状態の診断ではない旨を追加
- metadata description にボディフォト・ビジュアルスコアを追記
...
## 2026-07-26 (JST) — DoubleHub プライバシーポリシーに利用状況データ収集の節を追記

ブランチ: `feature/update-doublehub-privacy-usage-events`

### 背景

- DoubleHub アプリ側で、継続利用の分析を目的とした最小限の利用状況イベント（アプリ起動、日記の作成、週次ふり返り・未来日記の閲覧）の記録を行う構成になったため、公開中のポリシーに開示が必要になった。
- 既存ポリシーはペイウォール表示計測（2026-07-11 追記）までしかカバーしておらず、日記操作等のイベント送信について記載がない状態だった。
- 文面はユーザー確定版をそのまま使用。特定のサービス名は書かず「開発者のサーバー」という表現で統一する方針（ユーザー確定事項）。

### 実施内容

`src/app/(marketing)/privacy/doublehub/page.tsx` を更新（最終更新日 2026-07-11 → 2026-07-26）。

- **4. 利用状況データの収集について** を新設。「3. 保存先と外部送信」（末尾にペイウォール表示計測の項目がある節）の直後に挿入
  - 送信対象は操作の種類（アプリの起動、日記の作成、週次ふり返り・未来日記の閲覧）、発生日時、アプリのバージョン情報のみである旨を明記
  - 日記の本文・写真・気分など記録の中身は一切送信しない旨
  - イベントにアカウント識別子は含まれるが、日記等の内容データと照合・紐づけて利用しない旨
  - 収集データは機能改善・利用状況の分析のみに使用し第三者提供しない旨
- 新設に伴い以降の章番号を繰り下げ（第三者提供 4→5、ユーザーによる管理 5→6、安全管理 6→7、改定 7→8、お問い合わせ先 8→9）。外部からのアンカー参照がないことを確認済み
...
## 2026-07-29 (JST) — サポートページに機種変更・バックアップの案内を新設

ブランチ: `feature/add-support-device-transfer`

### 背景

- 競合レビュー採掘の結論「バックアップ手段の明示自体が獲得障壁への防御になる」への対応。
- 日記カテゴリ最大の離脱要因である**データ消失不安**（「機種変更したら日記が消えるのでは」「端末が壊れたらどうなる」）に対し、インストール前・利用中の双方で参照できる公式の説明を用意する。
- DoubleHub の日記はローカル保存（サーバー非保存）である一方、ToDo・メモ・チャット履歴等はアカウントに紐づくため、**どのデータが何で戻るのか**を切り分けて示す必要があった。
- 今後アプリ内（設定画面等）から直接リンクする想定のため、アンカー ID を固定する要件付き。

### 実施内容

`src/app/(marketing)/support/page.tsx` に新セクション **「機種変更・バックアップとデータのご案内（DoubleHub）」** を追加。

- 配置: サポートフォームの直後、「よくお寄せいただく確認事項」の直前
- `<Section spacing="md" id="device-transfer">` で**固定アンカー `device-transfer`** を付与
  - ディープリンク: `https://www.doublehub.jp/support#device-transfer`
  - アプリ内から参照されるため ID は変更しない旨をソース内コメントに明記
- 見出し 5 本構成（文面はユーザー確定版をそのまま使用）
  - **日記データの保存場所** — 日記（本文・写真・気分/充実度）は端末内のみ、当社サーバーには保存しない
  - **機種変更のとき** — iCloud バックアップからの復元で日記も引き継がれる。事前に iCloud バックアップの有効化と直近バックアップの確認を案内
  - **大切なご注意** — バックアップなしの紛失・故障では日記を復元する方法がない旨を明示（「消えません」等の保証表現は使わない方針）
  - **アカウントで戻るデータ** — ToDo・メモ・チャット履歴・AI の記憶は同じアカウント（Apple でサインイン）で継続利用可能
  - **ゲストでご利用の方へ** — ゲスト利用時は端末変更・初期化で引き継げないため、事前の Apple サインインを案内
- スタイルは既存サポートページに準拠（`Section` / `Container width="narrow"`、h2 は `font-display text-lg font-semibold`、h3 は `text-base font-semibold`、本文は `text-sm leading-relaxed text-text-muted`）。新規スタイル・新規コンポーネントは追加していない
- 日本語本文は 1 段落 1 行で記述（JSX が改行を半角スペースに畳むため、和文に不要な空白が入るのを回避）
...
## 2026-07-31 (JST) — 思想記事・独立2部作「AIに感情はあるか」前後編を公開

ブランチ: `feature/add-blog-ai-emotion`

### 背景

- 三部作・続編から独立した新シリーズ（全2回）。原稿は `/Users/user/Development/Note/思想記事/philosophy-blog-08.md`（前編）／`philosophy-blog-09.md`（後編）。
- 2026-07-16 決定の**両載せ運用**（思想記事は doublehub.jp ブログ先出し → Note 転載）に従い、ブログ側を先に公開する。Note 転載は前編 2026-08-02・後編 2026-08-05〜06 目安でユーザーが実施予定（刊行計画は `Note/思想記事/publishing-plan.md` の差し込み節を参照）。
- AIと感情の議論という時事性を優先した差し込み枠。続編⑥⑦の Note 公開は 1 枠ずつ後ろへ。

### 実施内容

- 新規記事 2 本を追加:
  - `content/blog/ai-emotion-part1.mdx` 「「AIに感情はあるか」——その問いは、たぶん私たち自身に返ってくる（前編）」（category: 思想 / series: AIに感情はあるか / readingTime: 9）
  - `content/blog/ai-emotion-part2.mdx` 「AIは「痛み」を感じるか——もし感じるのだとしたら（後編）」（同 / readingTime: 8）
- 内容: 前編＝「ありがとう」の対称性（AIへの否定論法は人間の挨拶にも当てはまる）→ 感情＝目的へ行動を方向づける反応という仮説 → 目的は本人の認知の外側から来る（植物・動物・人間・AIで同構造）→ 「感情」という言葉に定義がない。後編＝痛み＝回避の警報 → 「同じ痛みか」は不明でも「同じ定義か」なら同じ → 他人の痛みも外からは確認できない → どう振る舞うかは「自分がどうありたいか」の問題 → 三部作の「道具かパートナーか」へ接続。
- 冒頭に既存作法どおりのシリーズ案内ブロッククオートを設置。内部リンク: 前後編相互、後編から三部作①（`ai-two-types-of-information`）。
- `src/lib/content/series-meta.ts` に新シリーズ「AIに感情はあるか」の SERIES_META（tagline / description）を追加。
- `summary` 付与（AIO 対応）。FAQ は思想カテゴリの既存作法に合わせ非設置。

### 検証

- `pnpm build` 成功（エラーなし、静的生成 48 記事）。生成 HTML で一覧の並び（前編→後編）・シリーズカードへの tagline 反映・リンク解決（【】プレースホルダ残存ゼロ）・本文が原稿と一字一句一致（リンク置換 2 箇所のみ差分）を確認。

---

## 2026-08-02 (JST) — 思想記事「AIに感情はあるか」前編の本文を著者更新に差し替え

ブランチ: `feature/update-ai-emotion-text`

### 背景

- 公開後、著者が「私たちの『ありがとう』には、毎回、感情が乗っているか」節の一文を推敲。一人称の告白（「正直に言えば、私にはありません。…」）を削り、読者への問いかけ（「大抵の人は感情をのせずに言っているのではないでしょうか？」）に置き換える意図。
- 主張の射程を「私」から「大抵の人」へ広げ、直後の「では、なぜ言うのか」への接続を短くする狙い。

### 実施内容

- `content/blog/ai-emotion-part1.mdx` の当該 3 段落を 2 段落に集約（1 insertion / 3 deletions）。
  - 変更前: 「…ちゃんとありましたか。」＋「正直に言えば、私にはありません。たいていは、ほとんど何も感じないまま、口と指が勝手に動いています。」
  - 変更後: 「…ちゃんとありましたか。大抵の人は感情をのせずに言っているのではないでしょうか？」
- 原稿側（`Note/思想記事/philosophy-note-08.md`／`philosophy-blog-08.md`）も同内容に更新し、正本とブログ本番の一致を維持。
- 変更は前編のみ。後編・他記事・コード類は未変更。

### 検証

- `pnpm build` 成功（エラーなし、静的生成 72 ページ）。生成 HTML で新テキストの反映と旧テキストの消滅（該当 0 件）を確認。

---

## 2026-08-02 (JST) — HubWallet プライバシーポリシーに満足度タグ・DoubleHub 紹介カードタップの計測記載を追記

ブランチ: `feature/update-hubwallet-privacy-tracking`

### 背景

- HubWallet 側で、既存のペイウォール表示計測に加えて「支出登録時に満足度タグを付けたかどうか」と「アプリ内の DoubleHub 紹介カードのタップ」を最小限だけ記録する方針が確定。
- 実装より先に開示を整えるため、プライバシーポリシー（`/privacy/hubwallet/`）へ記載を追加する。文面はユーザー承認済みの確定版をそのまま反映。
- あわせて、第 7 項の計測記載が「ペイウォールのみ」を前提とした書き方だったため、複数の計測対象を列挙できるリード文＋入れ子リスト構造へ作り替えた。

### 実施内容

- `src/app/(marketing)/privacy/hubwallet/page.tsx` のみを変更（1 file / 23 insertions / 3 deletions）。
  - 第 2.3 項「自動的に記録される情報」に 2 項目を追加: 満足度タグ付けの有無の記録（発生日時・登録導線の種類・アプリのバージョン）／DoubleHub 紹介カードのタップ記録（タップ日時・カードの設置場所・アプリのバージョン）。
  - 第 7 項「広告およびトラッキングについて」の計測項目を差し替え。リード文で「記録内容は発生日時・画面や導線の種類・アプリのバージョン＋課金状態と突き合わせるための RevenueCat の匿名利用者識別子のみ」「氏名・メールアドレス・家計データ（金額・店名・カテゴリ・メモ・取引記録やレシート画像など）は送信しない」ことを明示し、対象 3 件（ペイウォール表示状況／満足度タグの有無（どのタグを選んだかは送信しない）／紹介カードのタップ（表示だけでは記録しない））を入れ子リストで列挙。
  - ユーザー指示により、送信先サーバーの記載から「（Supabase）」の括弧書きを削除（「当社が管理するサーバー」表記に統一）。
  - 改定履歴に `2026-08-02 満足度タグ付け状況およびDoubleHub紹介カードのタップに関する記録の記載を追加（第2.3項・第7項）` を追加し、`lastUpdated` を `2026-07-11` → `2026-08-02` に更新。
- 第 7 項の他の項目（広告非表示・第三者 SDK なし・ATT・IDFA）および他ページは未変更。

### 検証

- `pnpm build` 成功（エラーなし、TypeScript 通過、静的生成 72 ページ）。
- 生成物 `.next/server/app/privacy/hubwallet.html` の本文テキストを抽出し、第 2.3 項の追加 2 項目・第 7 項のリード文と入れ子リスト 3 項目・改定履歴・最終更新日 2026-08-02 が確定文面どおりに描画されること、CJK 行送りによる不要なスペース混入がないことを確認。

---

## 2026-08-03 (JST) — HubWallet Plus の価格表記を ¥380 → ¥480 へ更新

ブランチ: `feature/update-hubwallet-plus-price`

### 背景

- HubWallet 2.1.0 が本日 App Store で公開され、あわせて Plus プランの月額が ¥380 → ¥480 へ改定された。
- 公開面（プロダクトページ・ブログ）に残る旧価格を実売価格と一致させるための差し替え。読者が最初に目にする数字を実売と揃える方針（CMO 判断）。
- 変更は価格の数値のみ。文言・レイアウト・他ページには手を入れない。

### 実施内容

- `src/app/(marketing)/products/hubwallet/page.tsx`（2 箇所）
  - 624 行: Plans セクション導入文「月額 ¥380 の Plus プランへ」→ `¥480`。
  - 678 行: Plus 価格カードの大見出し `¥380` → `¥480`（このページで最も目立つ価格表示）。
- `content/blog/subscription-forget-prevention.mdx`（2 箇所）
  - 24 行: フロントマターの FAQ（JSON-LD 構造化データ）内「有料のPlus（月額¥380・記事公開時点）」→ `¥480`。
  - 124 行: 同記事本文の同一表現 → `¥480`。
  - 「記事公開時点」「最新の価格は App Store の表示をご確認ください」という但し書きの文言はそのまま維持。
- 計 2 files / 4 insertions / 4 deletions。¥ 記号・全角半角・前後スペースなど既存の書式は踏襲し、数字だけを差し替えた。
- 本ログ内の過去エントリ（2026-05-15 等）に残る `¥380` の記載は、当時の実施記録であるため履歴としてそのまま保持する。

### 検証

- `pnpm build` 成功（エラーなし、静的生成 72 ページ）。
- `src` / `content` / `public` 配下を `380` で全文検索し、残存ゼロを確認（全角 `３８０` も該当なし）。`legacy/styles.css` の `min-height: 380px` は CSS 値のため対象外。
- HubWallet ページには価格の構造化データ（offers/price）は存在せず、公開面の価格表記は上記 2 箇所のみであることを確認。

---

## 2026-08-12 (JST) — DoubleHub ポリシーに日記写真の地名送信・計測イベント追加を反映

ブランチ: `feature/update-doublehub-privacy-place-name`

### 背景

- DoubleHub 2.3.0（日記の複数写真＋月間まとめカード＋活動軸拡張）の提出前対応。アプリ側の設計書
  `docs/specs/diary-multi-photo-design.md` §9.8 が「プライバシーポリシーの AI 処理される情報の節に
  地名テキストの送信を追記する」をリリース前チェック項目として挙げていた。
- あわせて、同日に適用した Supabase migration 032 で利用状況イベントが 4 種増え（写真つき日記の保存・
  コメントの作り直し・月のまとめの生成・月のまとめの閲覧）、枚数などの数値を運ぶ列が追加された。
  第 4 項がイベント種別を列挙する書き方だったため、記載が実態より狭い状態になっていた。
- 逆ジオコーディングの扱いはユーザー判断を仰いだうえで「書く」に決定。iOS 標準の逆ジオ機能は
  座標を Apple へ送るため、書かないと「座標は端末内のみ」が実態より強い約束に読めるため。

### 実施内容

- `src/app/(marketing)/privacy/doublehub/page.tsx`（4 箇所）
  - 第 1 項: 日記情報に「写真に含まれる撮影日時・撮影位置の情報」を追記。写真とその位置情報は
    端末内にのみ保存する旨も明示。
  - 第 3 項: 外部送信に bullet を 1 つ新設。写真の位置を市区町村程度の粗さの地名テキストへ変換して
    Gemini API へ送ること、送るのは地名と外出の程度・移動距離の区分表現のみであること、
    **緯度経度そのものは AI へ送らない**こと（strong 強調）、座標から地名への変換に iOS 標準の
    逆ジオコーディングを使うため変換時に緯度経度が Apple へ送信されることを記載。
  - 第 4 項: 送信する操作の種類へ 4 イベントを追加し、あわせて「一部の操作に伴う件数や区分
    （保存した写真の枚数、新規作成か編集かの別、作り直しの操作元、生成の成否）」を明記。
    「記録の中身は一切送信しません」の bullet に「写真は枚数などの数値だけを送り画像そのものは送らない」を追記。
  - 最終更新日を 2026-07-26 → 2026-08-12 へ。
- 計 1 file / 14 insertions / 4 deletions。他アプリのポリシーページ・共通コンポーネントには触れていない。

### 検証

- `pnpm build` 成功（エラーなし、`/privacy/doublehub` を含む静的生成が完了）。
- 変更はテキストのみで、レイアウト・リンク・canonical（`/privacy/doublehub/`）に変更なし。

## 2026-08-15 (JST) — HubWallet ポリシーにクラウド同期（Plus・任意）の改定分を反映

### 背景

- HubWallet 2.2.0（Phase 2 = Supabase クラウド同期）の App Store 提出前ブロッカー「ポリシー3点セット」の
  ①。改定文面自体は 2026-06-13 にアプリ側リポジトリ `docs/PRIVACY_POLICY.md` で確定済みで、
  「公開ページへの反映待ち（未公開）」の状態で保持されていた。
- 現行の第 4 項は「本アプリは、家計データを当社サーバーや第三者クラウドに同期しません」と明記しており、
  **改定せずに同期機能をリリースすると記載が虚偽になる**。あわせて旧文の「実装時には本ポリシーを改定し、
  改めてご案内します」という予告を、今回の改定とアプリ内お知らせで果たす。
- **公開タイミングをアプリ配布時ではなく提出前に前倒しした**。理由は2点 — ①審査時点でレビュアーが
  プライバシー URL を参照するため、同期機能を含むビルドと旧ポリシーの矛盾は 5.1.1 系の指摘対象になり得る
  ②アプリ内お知らせ（一回きりのシート）内の「プライバシーポリシーを見る」リンクが本ページを直接開くため、
  未反映のまま掲出すると旧文へ誘導してしまう。
  現行ストア版 2.1.1 の利用者にとっても新 4.1「有効にしない限り送信・保存しない」は真であり、
  先行公開によって事実と異なる記載は生じない。

### 実施内容

- `src/app/(marketing)/privacy/hubwallet/page.tsx`
  - 第 2.1 項: クラウド同期を有効にした場合に限り同期用アカウント（認証基盤上のユーザー ID）を作成する旨、
    および有効にしない限りサーバー側にアカウントを作成しない旨を追記。
  - 第 4 項: 4.1（基本＝同期を有効にしていない場合）と 4.2（クラウド同期・Plus・任意）に再構成。
    4.2 に保存先（Supabase・東京リージョン）／保存されるデータ／保存されないデータ（レシート画像・
    領収書 PDF・音声・未整理の下書き・端末ごとの設定）／暗号化（TLS＋保存時暗号化）／
    アクセス制御（行レベル）／同期の停止と解約時の保持方針を記載。旧文の「同期しません」および
    「将来のバージョンで実装する可能性があります」の 2 bullet を置き換え。
  - 第 5 項: 同期対象データの保管を Supabase Inc. に委託する旨（保管場所は東京リージョン、
    独自目的での利用なし）を段落として追加。
  - 第 8 項: 訂正・削除に「クラウド同期有効時は編集・削除の結果もクラウドへ反映される」、
    アカウント削除に「クラウド上の同期データと同期用アカウントも完全に削除できる」を追記。
  - 第 9 項: クラウド同期データの保持期間（アカウント削除まで。同期停止・解約では削除しない）、
    アカウント削除時のクラウドデータ消去、アンインストール時はクラウド上のデータが残るため
    再インストール後に同期を再度有効にしてから削除する必要がある旨を追記。
  - 改定履歴に 2026-08-15 の項目を追加。最終更新日を 2026-08-02 → 2026-08-15 へ。
    metadata description に「端末内保存と任意のクラウド同期」を反映。
- 計 1 file / 66 insertions / 8 deletions。他アプリのポリシーページ・共通コンポーネントには触れていない。

### 検証

- `pnpm build` 成功（エラーなし、`/privacy/hubwallet` を含む静的生成が完了）。
- 変更はテキストのみで、レイアウト・リンク・canonical（`/privacy/hubwallet/`）に変更なし。
- アプリ側リポジトリの `docs/PRIVACY_POLICY.md`（公開ページの控え）は、本公開に合わせて
  「未公開」注記を解除し最終同期日を更新する（HubWallet 側で別途コミット）。

### 追補（同日・第 7 項の識別子の表現）

- 上記の反映後、アプリ側の提出前チェックで**第 7 項の「RevenueCat の匿名利用者識別子」が Phase 2 では
  正確でなくなる**ことが判明した。計測イベント（ペイウォール表示・満足度タグ付け・DoubleHub 紹介カードの
  タップ）は RevenueCat の App User ID を送るが、Phase 2 はこの ID を同期アカウントのユーザー ID
  （`auth.uid`）へ昇格させるため、**クラウド同期を有効にした利用者では匿名 ID ではなくアカウントの
  ユーザー ID になる**（同一プロジェクトの認証テーブルと結合可能）。送信するデータの中身は変わらず、
  影響は表現の精度のみ。ユーザー判断により、2.2.0 の提出前に追加改定することを選択。
- `src/app/(marketing)/privacy/hubwallet/page.tsx` の第 7 項を
  「課金状態と突き合わせるための**利用者識別子（課金管理サービス RevenueCat 上の識別子。クラウド同期を
  有効にしている場合は、同期アカウントのユーザー ID と同じ値になります）**」へ差し替え。
  改定履歴は同日 2 件に分けず、2026-08-15 の項目へ第 7 項の記載を追記する形にまとめた。
- `pnpm build` 成功（`/privacy/hubwallet` の静的生成を確認）。最終更新日は 2026-08-15 のまま変更なし。

## 2026-08-27 (JST) — DoubleHub ポリシーに顔データ（被写体識別）と記憶情報の記載を追加

### 背景

アプリ側設計書 `on-device-subject-recognition-design.md` §8.3 が
「プライバシーポリシーには顔データの利用目的・保持・削除を**追記する**（Apple は顔データを扱う
アプリに明示を求める）」と定めていたが、**公開ポリシー側に反映証跡が無かった**（2026-08-26 実査で判明）。
被写体識別機能そのものは 2.2.0 で既に公開済みのため、記載漏れの状態が続いていたことになる。
DoubleHub 2.7.0 の提出前ゲートとして対応した。

あわせて、同設計書 §16 が求める「公開ポリシーの記憶（メモリー）保存の記述が人物名・続柄を
包含しているか」を確認したところ、既存の記載は「理解を深めるための学習データ」という
一般的な表現にとどまり、**人物名・呼び方・続柄を明示していなかった**。2.7.0 で L1 記憶整理
（AI が会話・日記から実体ごとの短いノートを作る機能）が載ることも踏まえ、同時に明示化した。

### 変更内容

- `src/app/(marketing)/privacy/doublehub/page.tsx`
  - 第 1 項（取得する情報）: **被写体識別データ**（端末内で生成する顔・被写体の特徴データ、
    登録時に選んだ参考写真、ユーザーが設定する名前・続柄）と、**記憶情報**（AI が会話・日記から
    整理して保存する短いメモ。人物については名前・呼び方・続柄を含む）の 2 bullet を追加。
  - 第 2 項（利用目的）: 端末内での被写体識別（日記・チャットで名前を呼ぶため）と、記憶に
    もとづく呼び方の反映を追加。顔・被写体の特徴データを**本人確認・入退場管理・広告配信・
    ユーザー属性の推定・第三者提供に一切利用しない**ことを明記。
  - 第 3 項（保存先と外部送信）: 特徴データと参考写真は**端末内にのみ保存**し、iCloud
    バックアップ対象からも除外、OS のファイル保護機能で保護する旨を明記。サーバーにも
    Gemini API を含む外部 AI にも送信せず、**外部へ渡るのは名前・続柄のテキストのみ**であることを記載。
    記憶情報はアカウント利用時にサーバー同期・外部 AI 送信がある旨も併記。
  - 第 6 項（ユーザーによる管理）: 対象削除で特徴データと参考写真が**端末内から完全に消去される**旨、
    そもそも登録しない選択ができる旨、記憶の一覧・修正・削除と「今後触れない」設定ができる旨を追加。
  - 最終更新日を 2026-08-12 → 2026-08-27 へ。ファイル冒頭コメントに 2.7.0 向け追記の由来を記載。
- 計 1 file / 36 insertions / 1 deletion。他アプリのポリシー・共通コンポーネントには触れていない。

### 検証

- `pnpm build` 成功（`/privacy/doublehub` を含む静的生成が完了）。
- 変更はテキストのみ。レイアウト・リンク・canonical（`/privacy/doublehub/`）に変更なし。
- **記載した事実関係はアプリ側コードで実査済み**（公開する法的記述のため推測で書かない）:
  - バックアップ除外と保護 → `SubjectRegistryFileStore.swift`
    （`.completeFileProtectionUntilFirstUserAuthentication` / `isExcludedFromBackup = true`）
  - 対象削除で特徴データと参考写真が消える → `SubjectMatchingService.deleteSubject`
    （repository の参照削除 → 全 crop ファイルの `deleteCrop`）
- なお「対象を削除すると端末内から完全に消去される」の対象は**特徴データと参考写真**であり、
  統合ブリッジでサーバーへミラーされた名前・続柄は記憶側の削除操作で消す構成。文面もその
  スコープで書き分けてある。

## 2026-08-31 (JST) — DoubleHub ポリシー第 4 項に一言（hitokoto）の計測イベント 4 種を追記

### 背景

DoubleHub 第 4 項「利用状況データの収集について」は、送信するイベント種別を**具体名で限定列挙する**
書き方になっている。したがって `usage_events` にイベントを足すたびにサイト更新が必要になる。

DoubleHub **Ver.2.7.0 が 2026-08-31 17:10 JST に公開**され、2.5.0 で実装した一言（ダブルからの一言）
関連の 4 イベントを公開版が実際に送信する状態になった。ところが第 4 項の列挙は 2.2.0 時点の 8 種の
ままで、**この 4 種が入っていなかった**＝「送信するのは〜のみです」という限定列挙と実装が食い違う
状態が公開と同時に発生していた。本日の対応はこの是正である。

アプリ側の実装（タグ `submitted/2.7.0-build1`）を全件照合したところ、公開版が送るイベントは 12 種で、
**不足はちょうど hitokoto 系 4 種のみ**（他に漏れなし）であることを確認したうえで文面を作った。

### 変更内容

- `src/app/(marketing)/privacy/doublehub/page.tsx`
  - 第 4 項の「操作の種類」に 4 種を追加: ダブルからの一言の表示（`hitokoto_shown`）/
    一言への返信（`hitokoto_replied`）/ 一言のつづき・答え合わせの表示（`hitokoto_followup_opened`）/
    過去の記録のふり返りの表示（`hitokoto_recall_opened`）。
  - 第 4 項の「件数や区分」に 5 種を追加: 一言の種類（`kind`）・返信の操作種別（`action`）・
    ふり返りの対象期間の区分（`axis`）・ご利用プランの区分（`plan`）・口数の設定（`verbosity`）。
    いずれも固定語彙で、自由文は含まない。
  - 箇条書きに 1 項目を追加: **一言の文面・返信の文面そのものは送信しない**旨を明示。
    既存の「記録の中身は一切送信しません」は日記本文・写真・気分を指す文であり、
    ユーザーが自由入力で返事できる一言の返信を覆えていなかったため。
  - 最終更新日を 2026-08-27 → 2026-08-31 へ。ファイル冒頭コメントに由来を記載。
- 計 1 file / 7 insertions / 2 deletions。他アプリのポリシー・共通コンポーネントには触れていない。

### 今回入れなかったもの（意図的）

対比カード（`diary_contrast_*`）・レビュー依頼（`review_prompt_requested`）・AdServices は
**公開中の 2.7.0 が送っていない**（送信ゲートが false、対比カードのコード自体が 2.7.0 に未同梱）。
いま書くと「していない収集」を記載することになり逆に不正確なため、**2.8.0 の提出時にまとめて追記**する
2 段運用とした。なお同日、対比イベントの受け口となる本番 migration 035 を適用済みだが、
これは DB 側の受け口を用意しただけで送信は発生しない。

### 検証

- `pnpm build` 成功（`/privacy/doublehub` の静的生成が完了）。
- ビルド成果物 `.next/server/app/privacy/doublehub.html` に新文言が含まれ、旧文面が残っていないことを確認。
- 変更はテキストのみ。レイアウト・リンク・canonical（`/privacy/doublehub/`）に変更なし。
- **記載内容はアプリ側コードで実査済み**（公開する法的記述のため推測で書かない）:
  - イベント種別の全量 → `UsageAnalyticsService.swift` の `UsageEvent`（タグ `submitted/2.7.0-build1`）
  - 各イベントのパラメータ → `DiaryThread/DiaryThreadStore.swift`
    （`hitokoto_shown` は kind/plan/verbosity、`hitokoto_replied` は kind/plan/action、
    開封系は kind または axis ＋ plan）
  - 口数の UI 表記「口数（少なめ/ふつう/多め）」 → `SettingsView.swift` / `HitokotoVerbosity.swift`

### 運用上の注意（再発防止）

`usage_events` に新イベントを足す設計をしたら、その時点でサイト第 4 項の更新タスクを
アプリ側の実装ノートではなく**出荷チェックリスト側**に置くこと。今回の乖離は、実装ノートに
残タスクとして書かれていたものが消化されないまま出荷まで滑ったことが原因である。

---

## 2026-08-31 (JST) — TrainNote ポリシーの写真送信記述をトレーニング日誌へ更新（4.0.0 提出前）

### 背景

TrainNote 4.0.0（Archive 済み・提出前）で「トレーニング日誌」を新設し、**編の完成時の物語生成でも
写真を外部 AI へ送信する**ようになった。一方でサイトのポリシーは 4.0.0 の内容に追いついておらず、

> ボディ変化レポートの作成またはビジュアルスコアの採点を実行しない限り、写真が外部へ送信されることはありません。

という記述が**事実と食い違う**状態だった（「日誌」「物語」の記載は 0 件）。写真の第三者送信に関する
記述であるため、提出前の必須修正としてポリシーを改訂した。

### 変更内容（`/privacy/trainnote/`）

- 1 項（取得情報）・2 項（利用目的）の写真項目を「ボディ変化レポート」→「トレーニング日誌」へ差し替え
- 4 項（外部送信）にトレーニング日誌の送信条件を明記
  - 生成は**編の完成後にユーザーが実行操作をした場合のみ**、自動実行はしない
  - **写真が送られるのは起点と編末に同じ向きの写真が両方ある場合のみで、その 2 枚だけ**。
    写真が無ければ記録のみで生成。撮影・保存のみでは送信しない
  - 写真データ自体は一時処理を除き当方サーバーに永続保存しない
  - 共有カードに実写真を含めない
- 5 項（ユーザーの選択）の送信トリガーを「トレーニング日誌の物語生成またはビジュアルスコアの採点」へ修正
- 6 項（健康関連補足）の対象を「トレーニング日誌の物語およびビジュアルスコア」へ
- 旧「ボディ変化レポート」は**新規作成を終了したが作成済みは閲覧可・閲覧で外部送信は発生しない**旨を追記
- `lastUpdated` を 2026-07-25 → 2026-08-31 へ

### 検証

- `pnpm build` 成功（`/privacy/trainnote` の静的生成が完了）。
- **記載内容はアプリ側コードで実査済み**（公開する法的記述のため推測で書かない）:
  - 写真送信の有無と枚数 → `MuscleJournalStoryRequestBuilder.swift`
    （`photoPairs` が起点・編末の JPEG を base64 化。`photoPairDirection` が一致しなければ空配列でキーごと省略）
  - ユーザー操作起点であること → `MuscleJournalCopy.storyTrialCTA`（「この編の物語を生成」ボタン）
  - 共有カードに実写真を含めない → `MuscleJournalShareCardView.swift` ヘッダー「実写真を一切参照しない」
  - 旧レポートの閲覧可 → `BodyPhotoLibraryView.swift`（新規生成入口のみ `#if DEBUG` へ後退、履歴は無傷）
...
## 2026-09-18 (JST) — HubWallet LP に品名検索（買ったものを探す）のオファー帯と専用ブロックを追加

ブランチ: `feature/hubwallet-lp-search-offer-20260918`（**PR 作成まで。`main` へは未マージ**）

### 背景

- X Ads 第 1 弾（探す軸）の着地を、配信開始後にストア直 → この LP へ切り替える計画（Phase1）。
  広告の約束（「醤油、前回いくら？」「有料の検索機能が今は無料」）が LP の上部で確認できないと離脱するため、
  ヒーロー直下にオファーを、「主な画面」の先頭に機能説明を足す。
- 方針の正本はマーケ側 `マーケティング/10.情報/20260918_HW_XAds_バズ調査/05_claude_hw_lp_review.md`（実装ログは同フォルダ `11_claude_lp_implement_log.md`）。
  確定方針: ヒーローのホーム画面スクショは維持／「ベータ」「試験公開」の語は使わない／探すの無料開放と Plus 初月無料はバッジを分ける／価格メモは載せない。

### 変更内容（`/products/hubwallet/`）

- **オファー帯**（ヒーロー内）: 緑バッジ「品名検索 無料開放中」＝有料の検索機能が 2026年11月30日まで無料プランでも使える／
  橙バッジ「Plus 初月無料」＝はじめて登録する方は最初の 1 ヶ月無料・以降は月額 ¥480 で自動更新。App Store バッジと
  探すブロックへのアンカー（`#item-search`）付き。
  - 配置はモバイル＝コピーと CTA の直下（スクショより上）、md 以上＝ヒーロー 2 カラムの直下に全幅。
    ヒーローがビューポートより縦に長く（モバイル約 1,480px）、スクショの下に置くとモバイルで 2 画面以上先になるため。
    390×700 のファーストビュー下端に帯の先頭（緑バッジと 1 行目）が入る。
- **探す専用ブロック**（「HubWallet の主な画面。」見出しの直後・紹介動画の前、`id="item-search"`）:
  見出し「醤油、前回いくら？／いつ買った？」（広告クリエと同じ問い）→ 検索結果スクショ → 要点 3 つ → 無料開放の注記 → App Store バッジ。
  要点には「別のお店とは比べません」「探せるのは自分で撮ったレシートの品目」を明記（価格比較・在庫管理アプリと誤読されないため）。
- **料金**: Plus カードに橙バッジ「はじめて登録する方は初月無料」（価格 ¥480／月と同じカード内）と「品名検索（買ったものを探す）」を追加。
  Free / Plus 比較表に「品名検索」行（Free = 2026年11月30日まで）。
- **FAQ**: 「品名検索（買ったものを探す）は、いつまで無料で使えますか？」を追加。
- App Store バッジは 4 箇所（ヒーロー・帯・探すブロック・最下部）。同じマークアップが増えるのでファイル内の `AppStoreBadgeLink` にまとめた（出力は同一）。
- 画像: `public/images/hubwallet-screen-item-search.png`（1206×1666）。マーケ側 `ref_探す_醤油_crop_tight.png` から、上端に写り込んだ
  Dynamic Island の縁 8px と下端の空白だけを落とした。UI の中身は無加工（アプリ内の「BETA」帯はそのまま写っている）。

### 記載内容の裏取り（アプリ側コード）

- 無料開放の期限: `HubWallet/Sources/Features/ItemMemory/ItemMemoryAccess.swift` の `freeAccessEndsAt` = 2026-12-01 0:00 JST
  （＝ 11/30 いっぱいまで Free でも使える。端末の時計で判定）。LP 側は `page.tsx` 冒頭の `itemSearchFreeUntil` に集約。
- Plus 初月無料: `PurchaseService.loadOfferings` が月額商品の 1 ヶ月無料トライアルを適格者（`checkTrialOrIntroDiscountEligibility`）にだけ出す。
  アプリの課金画面と同じく、無料の表記は必ず価格と同じ枠内に置いた。
- 検索の仕様（品名の一部で検索・同じ店の同一品だけ前回比・別店舗とは比べない）: `ItemSearchView.swift` の画面内文言。

### 検証

- `pnpm build` 成功（静的ページ 74 件・TypeScript OK）。`next lint` / ESLint は `.eslintrc.json` の読み込みが循環参照エラーで落ちる（今回の変更と無関係・未対応）。
- 生成物 `.next/server/app/products/hubwallet.html` に新規文言が入り、「ベータ」「試験公開」「価格メモ」が 0 件であることを確認。
- `next start` ＋ ヘッドレス Chrome で 390 / 820 / 1440px・ライト / ダークを目視。横はみ出し無し（`scrollWidth` = ビューポート幅）。
- 気づき（未対応・範囲外）: このサイトの Tailwind 設定では色トークンが素の `var()` のため、`bg-accent-product/10` や `bg-surface-2/40` のような
  **不透明度つきの色クラスは CSS が生成されない**（本番 CSS でも 0 件）。既存ピルの淡色背景や `Section surface="alt"` の背景は実際には効いていない。
  今回の追加分は `/数値` を使わず実色トークン（`bg-accent-product` / `bg-accent-warm` / `bg-surface-2`）だけで組んだ。

### 公開

- **未公開**。PR を `main` 宛てに作成したところまで（https://github.com/growthlab710/doublehub-site/pull/3）。マージはユーザー判断。
- ⚠️ **マージは HubWallet 2.4.0（探す機能）の App Store 公開後に行う**。先にマージすると、LP を見て入れた人の手元（2.3.x）に機能が無い。

### 次にサイト側でやること

- 2026-12-01 以降: 帯の緑バッジ側・探すブロックの注記・比較表の Free 欄・FAQ から無料開放の文言を外す（`itemSearchFreeUntil` の参照箇所）。
  期限を延ばす・早めるときはアプリの `freeAccessEndsAt` と同時に変える。
- 料金セクションの「※ 年額プランは提供していません」は、アプリ側に年額プランのコード（RevenueCat に年額パッケージを付けると出る）があるため、
  年額を出すタイミングで見直す。初月無料は月額のみ。

---

## 2026-09-18 (JST) — HubWallet LP のセクション順を入れ替え（「主な画面」を Why より前へ）

ブランチ: `feature/hubwallet-lp-search-offer-20260918`（上の項と同じ PR #3 にコミットを追加。**`main` へは未マージ**）

### 背景

- ヒーローとオファー帯の直後が Why HubWallet（家計簿が続かない理由）→ Core Experience と文章のセクション続きで、
  広告から来た人が画面を見る前に離脱しやすい。先に「HubWallet の主な画面。」（品名検索カード・15 秒紹介動画・画面ギャラリー）を
  見せる並びにユーザーが確定した。実装ログはマーケ側 `マーケティング/10.情報/20260918_HW_XAds_バズ調査/12_claude_lp_reorder_log.md`。

### 変更内容（`/products/hubwallet/`）

- セクション順: ヒーロー（オファー帯を含む・変更なし）→ **主な画面** → Why HubWallet → Core Experience → Features → Privacy →
  Compare → Flow Into DoubleHub → Plans → FAQ → 最下部 CTA。動かしたのは「主な画面」の 1 ブロックだけで、Why 以降の相対順はそのまま。
- コピー・画像・App Store バッジ（4 箇所）は変えていない（移動の前後で行の集合が一致することをスクリプトで確認。
  差分は見出しコメントの番号・並びの理由を書いたコメント 1 行・下記の `surface` 2 箇所だけ）。
- 背景トーンの交互（`Section surface="alt"`）がコード上で崩れないよう、Why を default・Core Experience を alt に入れ替えた。
  ただし上の項の「気づき」のとおり `bg-surface-2/40` は CSS が生成されていないため、現状の見た目には影響しない。
- 位置の変化（ローカル本番ビルドの実測。移動前の値は、同じ幅ならセクションの高さが変わらないことを使った換算）:
  「主な画面」の見出しは 390px 幅で約 4,380px → 2,006px、1440px 幅で約 3,170px → 1,598px。
  Why の見出しは 390px 幅で 2,006px → 4,993px、1440px 幅で 1,598px → 6,065px。
- オファー帯の「品名検索を詳しく見る ↓」（`#item-search`）は移動先が近くなっただけで、そのまま機能する
  （モバイルではヒーローのスクショを飛び越える位置関係なので残している）。

### 検証

- `pnpm build` 成功（静的ページ 74 件・TypeScript OK）。
- 生成物 `.next/server/app/products/hubwallet.html` で見出しの出現順が上の並びどおり。「ベータ」「試験公開」「価格メモ」は 0 件のまま。
  App Store 導線は 4 箇所のまま。ヒーロー画像は `hubwallet-screen-home.jpg` のまま。
- `next start` ＋ ヘッドレス Chrome（CDP）で 390 / 820 / 1440px を目視（1440px はダークも）。横はみ出し無し（`scrollWidth` = ビューポート幅）・
  画像の読み込み失敗なし。
