...
> **詳細は [07-delivery-and-handover.md](./07-delivery-and-handover.md) §7.4 を必読**。以下はサマリ。

Day 5 完了時に作成する `docs/web-renewal/HANDOVER.md` には以下を必ず含める：

- 作業サマリ（動作検証済み / 未実装、既知のバグ、Lighthouse スコア）
- **【最重要】依頼者向けファイル置き換え手順**（依頼者はローカルで手作業で置き換える。迷わず実行できる具体性で記述）
- 置き換え時の保持対象リスト（`.git/`, `CNAME`, `robots.txt`, `google*.html`, `manifest.json`, favicon, docs/web-renewal/ 等）
- トラブルシュート（ビルドエラー時の診断、表示されない時の原因候補）
- Cloudflare Pages プレビュー URL と認証方法
- 後続セッション（Claude）への引き継ぎ事項（Supabase ダッシュボード設定、本番環境変数、DNS 切替、iOS 側対応）
- 要確認事項リスト全体
- 推奨される次のステップ（所要時間付き）
- 作業完了チェックリスト（エージェント自己申告）
